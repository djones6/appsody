...
func initAppsody(stack string, template string, config *initCommandConfig) error {
	...
	err = CheckPrereqs()
	if err != nil {
		rootConfig.Warning.logf("Failed to check prerequisites: %v\n", err)
	}

	//err = index.getIndex()

	indices, err := repos.GetIndices(rootConfig)
	...
	if stack != "" {
		...
		rootConfig.Debug.log("Attempting to locate stack ", projectType, " in repo ", repoName)
		index = indices[repoName]
		projectFound := false
		stackFound := false

		if strings.Compare(index.APIVersion, supportedIndexAPIVersion) == 1 {
			rootConfig.Warning.log("The repository .yaml for " + repoName + " has a more recent APIVersion than the current Appsody CLI supports (" + supportedIndexAPIVersion + "), it is strongly suggested that you update your Appsody CLI to the latest version.")
		}
		if len(index.Projects[projectType]) >= 1 { //V1 repos
			...
			rootConfig.Debug.log("Project ", projectType, " found in repo ", repoName)
		...
		for _, stack := range index.Stacks {
			if stack.ID == projectType {
				stackFound = true
				rootConfig.Debug.log("Stack ", projectType, " found in repo ", repoName)
				URL := ""
		...
		if noTemplate || config.overwrite {
			...
			proceedWithTemplate, err = isFileLaydownSafe(rootConfig, dir)
			if err != nil {
				return err
			}
		}
		...
		if !config.overwrite && !proceedWithTemplate {
			rootConfig.Error.log("Non-empty directory found with files which may conflict with the template project.")
			rootConfig.Info.log("It is recommended that you run `appsody init <stack>` in an empty directory.")
			rootConfig.Info.log("If you wish to proceed and possibly overwrite files in the current directory, try again with the --overwrite option.")
		...
		rootConfig.Info.logf("Downloading %s template project from %s", projectType, projectName)
		filename := filepath.Join(dir, projectType+".tar.gz")

		err = downloadFileToDisk(rootConfig, projectName, filename, config.Dryrun)
		if err != nil {
			return errors.Errorf("Error downloading tar %v", err)

		}
		if inputTemplateName != "none" {
			rootConfig.Info.log("Download complete. Extracting files from ", filename)
		} else {
			rootConfig.Info.log("Download complete. Do not unzip the template project. Only extracting .appsody-config.yaml file from ", filename)
		}

		//if noTemplate
		errUntar := untar(rootConfig, filename, noTemplate, config.overwrite, config.Dryrun)

		if config.Dryrun {
			rootConfig.Info.logf("Dry Run - Skipping remove of temporary file for project type: %s project name: %s", projectType, projectName)
		} else {
			err = os.Remove(filename)
			if err != nil {
				rootConfig.Warning.log("Unable to remove temporary file ", filename)
			}
		}
		if errUntar != nil {
			rootConfig.Error.log("Error extracting project template: ", errUntar)
			rootConfig.Info.log("It is recommended that you run `appsody init <stack>` in an empty directory.")
			rootConfig.Info.log("If you wish to proceed and overwrite files in the current directory, try again with the --overwrite option.")
	...
	if template == "" {
		rootConfig.Info.logf("Successfully initialized Appsody project with the %s stack and the default template.", stack)
	} else if template != "none" {
		rootConfig.Info.logf("Successfully initialized Appsody project with the %s stack and the %s template.", stack, template)
	} else {
		rootConfig.Info.logf("Successfully initialized Appsody project with the %s stack and no template.", stack)
	}
...
func install(config *initCommandConfig) error {
	rootConfig := config.RootCommandConfig
	rootConfig.Info.log("Setting up the development environment")
	...
	rootConfig.Debug.logf("Setting up the development environment for projectDir: %s and platform: %s", projectDir, platformDefinition)
	...
	if err != nil {
		...
		rootConfig.Warning.log("The stack init script failed: ", err)
		rootConfig.Warning.log("Your local IDE may not build properly, but the Appsody container should still work.")
		rootConfig.Warning.log("To try again, resolve the issue then run `appsody init` with no arguments.")
		os.Exit(0)
	}
	return nil
}

func untar(rootConfig *RootCommandConfig, file string, noTemplate bool, overwrite bool, dryrun bool) error {

	if dryrun {
		rootConfig.Info.log("Dry Run - Skipping untar of file:  ", file)
	} else {
		untarDir := filepath.Dir(file)
		if !overwrite && !noTemplate {
			err := preCheckTar(rootConfig, file, untarDir)
			if err != nil {
				return err
			}
		}
		...
		for {
			...
			rootConfig.Debug.log("Untar creating ", filename)
...
func isFileLaydownSafe(rootConfig *RootCommandConfig, directory string) (bool, error) {

	safe := true
	files, err := ioutil.ReadDir(directory)
	if err != nil {
		rootConfig.Error.logf("Can not read directory %s due to error: %v.", directory, err)
		return false, err
	...
	for _, f := range files {
		...
		if !whiteListed {
			safe = false
			rootConfig.Debug.logf("%s file exists and is not safe to extract the project template over", f.Name())
		} else {
			rootConfig.Debug.logf("%s file exists and is safe to extract the project template over", f.Name())
		}
	}
	if safe {
		rootConfig.Debug.log("It is safe to extract the project template")
	} else {
		rootConfig.Debug.log("It is not safe to extract the project template")
	}
	return safe, nil
...
func preCheckTar(rootConfig *RootCommandConfig, file string, untarDir string) error {
	preCheckOK := true
	fileReader, err := os.Open(file)
	if err != nil {
		return err
		...
		if header == nil {
			...
			if inWhiteList(header.Name) {
				...
				if err == nil {
					if !fileInfo.IsDir() {
						preCheckOK = false
						rootConfig.Warning.log("Conflict: " + header.Name + " exists in the file system and the template project.")
...
func extractAndInitialize(config *initCommandConfig) error {
	...
	if !config.RootCommandConfig.Buildah { //We can skip extract in some cases
		...
		rootConfig.Debug.log("Attempting to run ", bashCmd, " on image ", stackImage, " with options: ", cmdOptions)
		...
		if err != nil {
			rootConfig.Debug.log("Failed to run the find command for the ", scriptFileName, " on the stack image: ", stackImage)
			return fmt.Errorf("Failed to run the docker find command: %s", scriptFindOut)
		}

		if scriptFindOut == "" {
			rootConfig.Debug.log("There is no initialization script in the image - skipping extract and initialize")
			return nil
		}
	}
	...
	if !config.Dryrun {
		...
		rootConfig.Info.log("Dry Run skipping extract.")
	}
	...
	if scriptExists && err == nil { // if it doesn't exist, don't run it
		rootConfig.Debug.log("Running appsody_init script ", scriptFile)
		err = execAndWaitWithWorkDirReturnErr(rootConfig, scriptFile, nil, rootConfig.InitScript, workdir, config.Dryrun)
		if err != nil {
			return err
		}
	}
