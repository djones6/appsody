...
func GetGitInfo(config *RootCommandConfig) (GitInfo, error) {
	var gitInfo GitInfo
	version, vErr := RunGitVersion(config, false)
	if vErr != nil {
		return gitInfo, vErr
	}
	if version == "" {
	...
	output, gitErr := RunGit(config, kargs, config.Dryrun)
	if gitErr != nil {
	...
	if gitInfo.Upstream != "" {
		gitInfo.RemoteURL, gitErr = RunGitConfigLocalRemoteOriginURL(config, gitInfo.Upstream, config.Dryrun)
		if gitErr != nil {
...
func RunGitConfigLocalRemoteOriginURL(config *RootCommandConfig, upstream string, dryrun bool) (string, error) {
	config.Info.log("Attempting to perform git config --local remote.<origin>.url  ...")

	upstreamStart := strings.Split(upstream, "/")[0]
	kargs := []string{"config", "--local", "remote." + upstreamStart + ".url"}
	remote, err := RunGit(config, kargs, dryrun)
	if err != nil {
		return remote, err
	}
...
func RunGitGetLastCommit(URL string, config *RootCommandConfig) (CommitInfo, error) {
	...
	commitStringInfo, gitErr := RunGit(config, kargs, config.Dryrun)
	if gitErr != nil {
...
//RunGitVersion
func RunGitVersion(config *RootCommandConfig, dryrun bool) (string, error) {
	kargs := []string{"version"}
	versionInfo, gitErr := RunGit(config, kargs, dryrun)
	if gitErr != nil {
		return "", gitErr
	}
...
func RunGit(config *RootCommandConfig, kargs []string, dryrun bool) (string, error) {
	kcmd := "git"
	if dryrun {
		config.Info.log("Dry run - skipping execution of: ", kcmd, " ", strings.Join(kargs, " "))
		return "", nil
	}
	config.Info.log("Running git command: ", kcmd, " ", strings.Join(kargs, " "))
	...
	config.Debug.log("Command successful...")
	result := string(kout[:])
